...
| is_hit_labour_day_activity | bool | 是否命中劳动节活动 | |
| is_story                | bool    | 是否可以在 动态视频Story Mode 展示    |                                                              |
...
| is_story_play           | bool    | 是否为 动态视频Story Mode 视频?        |                                                              |
...
- `is_upower_preview` 表示该充电专属视频是否支持试看
- `is_upower_exclusive_with_qa` 表示该充电专属视频是否附带专属问答能力
- 根据抓包样本，部分普通视频页面的内联状态中也可能出现充电状态字段，因此不要只凭通用充电状态去判断当前视频是否专属
- 若需要进一步判断该视频要求的充电档位，建议结合[播放器信息](player.md#web-播放器信息)中的`elec_high_level.privilege_type`
- 若需要在空间投稿列表场景判断某稿件是否为充电专属，建议结合[用户空间相关](../user/space.md#查询用户投稿视频明细)中的`is_charging_arc`、`elec_arc_type`、`elec_arc_badge`
...
| 字段    | 类型  | 内容       | 备注                              |
|-------|-----|----------|---------------------------------|
| judge | num | 结论判断状态 | 0：无结论<br />1：有结论（视频存在AI总结） |

**示例：**

查询视频`BV1cUdEBZEcX`的结论判断状态

```shell
curl -G 'https://api.bilibili.com/x/web-interface/view/conclusion/judge' \
--data-urlencode 'bvid=BV1cUdEBZEcX' \
--data-urlencode 'cid=37596431790' \
--data-urlencode 'up_mid=456664753'
```
...
> https://api.bilibili.com/x/web-interface/view/uplikeimg
...
| 字段 | 类型 | 内容 | 备注 |
| ---- | ---- | ---- | ---- |
| up_like_img | str | UP主点赞图 | |

**示例：**

查询视频`aid=116422940300634`的UP主点赞图片

```shell
curl -G 'https://api.bilibili.com/x/web-interface/view/uplikeimg' \
--data-urlencode 'aid=116422940300634' \
--data-urlencode 'vmid=456664753'
```
...
> https://api.bilibili.com/x/web-interface/view/cards
...
| is_interact_display | bool | 是否显示交互   |                       |
| is_follow_display  | bool | 是否显示关注   |                       |
| title              | str  | 标题           |                       |
| subtitle           | str  | 副标题         |                       |

**示例：**

查询视频`BV1zbcDzbEn8`的卡片信息

```shell
curl -G 'https://api.bilibili.com/x/web-interface/view/cards' \
--data-urlencode 'bvid=BV1zbcDzbEn8' \
--data-urlencode 'aid=116221127163942' \
--data-urlencode 'cid=36667722519' \
...
| 字段       | 类型  | 内容           | 备注                  |
| ---------- | ----- | -------------- | --------------------- |
| attention  | bool  | 是否关注UP主   | true：已关注<br />false：未关注 |
| favorite   | bool  | 是否收藏视频   | true：已收藏<br />false：未收藏 |
| season_fav | bool  | 是否收藏合集   | true：已收藏<br />false：未收藏 |
| like       | bool  | 是否点赞视频   | true：已点赞<br />false：未点赞 |
| dislike    | bool  | 是否点踩视频   | true：已点踩<br />false：未点踩 |
| coin       | num   | 投币数量       |                       |

**示例：**

查询视频`BV1zbcDzbEn8`的用户关系状态

```shell
curl -G 'https://api.bilibili.com/x/web-interface/archive/relation' \
--data-urlencode 'bvid=BV1zbcDzbEn8' \
--data-urlencode 'aid=116221127163942'
```
