...
| 参数名    | 类型 | 内容             | 必要性 | 备注                          |
| --------- | ---- | ---------------- | ------ | ----------------------------- |
| position  | num  | 广告位id（可多个） | 必要   | 多个用`&`重复传递，如`position=52&position=53` |
...
| ---------------- | ----- | ------------------ | ---- |
| list             | array | 广告素材列表       |      |
| list_v2          | array | 广告素材列表(v2)   |      |
| vip_login_coupon | obj   | 大会员登录优惠券信息 |    |

`data`中的`list_v2`数组中的对象：

| 字段          | 类型 | 内容           | 备注 |
| ------------- | ---- | -------------- | ---- |
| id            | num  | 素材id         |      |
| position      | num  | 广告位id       |      |
| track_params  | str  | 追踪参数       |      |
| theme         | str  | 主题标识       |      |
| title_list    | str  | 标题列表       |      |
| button_list   | str  | 按钮列表       |      |
| click_target  | str  | 点击目标       |      |
| extra_params  | str  | 额外参数       |      |
| weight        | num  | 权重           |      |
        ...
        "list_v2": [
            {
                "id": 18634,
                "position": 52,
                "track_params": "exp_group_tag=def; exp_tag=def; material_type=3; position_id=52",
                "theme": "TOP_GUIDE_VIP_ENTRANCE",
                "title_list": "  ",
                "button_list": "",
                "click_target": "jump_type=0",
                "extra_params": "is_rta=0",
                "weight": 21
            }
        ],
        "vip_login_coupon": {
...
> https://api.bilibili.com/x/vip/ads/material/report
...
> https://api.bilibili.com/x/vip/order/token/get
...
```json
{
    ...
    "ttl": 1,
    "data": {
        "expire_at": 1776325585,
        "url": "https://big.bilibili.com/mobile/payMiddleWare?type=0&token=91998665260416153625",
        "token": "91998665260416153625",
        "money": 25
    }
}
```
