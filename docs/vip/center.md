...
> https://api.bilibili.com/x/vip/web/vip_center/info
...
| 参数名     | 类型 | 内容         | 必要性 | 备注                                 |
| ---------- | ---- | ------------ | ------ | ------------------------------------ |
| module_id  | num  | 模块ID       | 非必要 | 默认700001                           |
| platform   | str  | 平台标识     | 非必要 | pc                                   |
| build      | num  | 构建版本     | 非必要 | 默认为0                              |
...
| top_banner      | obj   | 顶部横幅       |      |
| user            | obj   | 用户信息       |      |
| welfare_activity| obj   | 福利活动       |      |
| taps            | array | 分类标签       |      |
| face            | null  | （？）         |      |

`data`中的`top_banner`对象：

| 字段名  | 类型  | 内容       | 备注 |
| ------- | ----- | ---------- | ---- |
| banners | array | 横幅列表   |      |

`top_banner`中的`banners`数组：

| 字段名        | 类型 | 内容           | 备注                    |
| ------------- | ---- | -------------- | ----------------------- |
| cover         | str  | 封面图片       |                         |
| small_cover   | str  | 小封面图片     |                         |
| title         | str  | 标题           |                         |
| logo          | str  | Logo图片       |                         |
| link          | str  | 链接           |                         |
| season_status | num  | 剧集状态       |                         |
| season_id     | num  | 剧集ID         |                         |
| play_title    | str  | 播放标题       |                         |
| user_status   | num  | 用户状态       |                         |
| season_type   | num  | 剧集类型       | 1:番剧 2:电影 3:纪录片 4:国创 5:电视剧 7:综艺 |
...
| account           | obj  | 账号信息           |      |
| vip_info          | obj  | VIP信息            |      |
| tv_vip_info       | obj  | 电视VIP信息        |      |
| overdue_explain   | obj  | 过期说明           |      |
| wallet            | obj  | 钱包信息           |      |
| open_entrance     | obj  | 开通入口           |      |
| vip_surplus_seconds| num | VIP剩余秒数        |      |
| tv_surplus_seconds| num  | 电视VIP剩余秒数    |      |
| is_auto_renew     | bool | 是否自动续费       |      |
| is_tv_auto_renew  | bool | 电视VIP自动续费    |      |
| vip_keep_time     | num  | VIP保持时间        |      |
| is_sign_surplus   | bool | （？）             |      |
| notice            | obj  | 通知信息           |      |
...
```shell
curl 'https://api.bilibili.com/x/vip/web/vip_center/info?module_id=700001&platform=pc' \
  -b 'SESSDATA=xxx'
```
...
```json
{
  ...
  "data": {
    "top_banner": {
      "banners": [
        {
          "cover": "https://i0.hdslb.com/bfs/bangumi/image/571afca63464cc5eb9edca8a9dd5c63dac3d2918.png",
          "small_cover": "https://i0.hdslb.com/bfs/bangumi/image/b6446df705955a6f68dc53a8c6ad65736e939323.png",
          "title": "弹珠汽水瓶里的千岁同学",
          "logo": "https://i0.hdslb.com/bfs/activity-plat/static/20221019/a5480f18ac08c1e30876e3fde84db784/NY19YUaSZS.png",
          "link": "https://www.bilibili.com/bangumi/play/ep3602621",
          "season_status": 13,
          "season_id": 112481,
          "play_title": "",
          "user_status": 0,
    ...
    "user": {
      ...
      "open_entrance": {
        "vip_button_tip": "成为大会员",
        "corner_marker": "低至0.4元/天",
        "gift_button_tip": "赠送"
      }
    },
    "taps": [
      {
        "oid": "{season_type:0,style_id:0,day:3,rank_type:5}",
        "rank_id": 279,
        "rank_title": "会员"
      },
      {
...
> https://api.bilibili.com/x/vip/vip_center/guesslike
...
| 参数名   | 类型 | 内容         | 必要性 | 备注                    |
| -------- | ---- | ------------ | ------ | ----------------------- |
| exp      | num  | 实验参数     | 非必要 | 默认1                   |
| fnver    | num  | 功能版本     | 非必要 | 默认0                   |
| fnval    | num  | 功能值       | 非必要 | 默认2000                |
| qn       | num  | 清晰度       | 非必要 | 默认0                   |
| fourk    | num  | 是否4K       | 非必要 | 默认1                   |
| net_type | num  | 网络类型     | 非必要 | 默认2                   |
...
| badge_info           | obj  | 徽章信息       |      |
| partition            | str  | 分区           |      |
| first_ep_id          | num  | 第一集ID       |      |
| season_type          | num  | 剧集类型       |      |
| season_status        | num  | 剧集状态       |      |
...
> https://api.bilibili.com/x/vip/web/vip_center/season
...
| 参数名         | 类型 | 内容           | 必要性 | 备注                    |
| -------------- | ---- | -------------- | ------ | ----------------------- |
| index_type     | num  | 索引类型       | 非必要 | 102:推荐 101:热门       |
| order          | num  | 排序方式       | 非必要 | 2:综合 4:评分           |
| season_status  | str  | 剧集状态       | 非必要 | 2,4,6 等                |
| is_finish      | num  | 是否完结       | 非必要 | 0:连载 1:完结           |
| type           | num  | 类型           | 非必要 | 默认1                   |
| page           | num  | 页码           | 非必要 | 默认1                   |
| page_size      | num  | 每页数量       | 非必要 | 默认30                  |
| fnver          | num  | 功能版本       | 非必要 | 默认0                   |
| fnval          | num  | 功能值         | 非必要 | 默认2000                |
| qn             | num  | 清晰度         | 非必要 | 默认0                   |
| fourk          | num  | 是否4K         | 非必要 | 默认1                   |
...
> https://api.bilibili.com/x/vip/vip_center/hotlist
