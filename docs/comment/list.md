...
| giveup_input_text | str | 放弃评论提示文字 | |
| screenshot_icon_state | num | 截图图标状态 | |
| upload_picture_icon_state | num | 上传图片图标状态 | |
| empty_page | null | 空页面配置 | |
| show_type | num | 显示类型 | |
| show_text | str | 显示文本 | |
| disable_jump_emote | bool | 禁止跳转表情? | |
| enable_charged | bool | 启用充电相关? | |
| enable_cm_biz_helper | bool | 启用广告助手? | |
| preload_resources | null | 预加载资源 | |
...
| replies      | 禁用时：null<br />正常时：array | 评论列表 |                  |
| top          | obj                       | 置顶信息 |  |
| top_replies | 禁用时：null<br />正常时：array | 置顶评论列表 | 项为[评论条目对象](readme.md#评论条目对象) |
...
| mode         | num   | 排序方式       |                         |
| mode_text    | str   | 排序方式文本名称 | |
| support_mode | array | 支持的排序方式 |                         |
| name         | str   | 评论区类型名   |                         |
| pagination_reply | obj | 分页偏移信息 | 见下 |
| session_id   | str   | 空 |  |
...
> https://api.bilibili.com/x/v2/reply/subject/interaction-status
...
| 字段             | 类型 | 内容           | 备注 |
| ---------------- | ---- | -------------- | ---- |
| up_reply_selection | obj | UP主评论精选设置 |      |
| up_reply         | obj  | UP主评论设置   |      |
| up_dm            | obj  | UP主弹幕设置   |      |
| can_modify       | bool | 是否可以修改   |      |

`data`中的`up_reply_selection`、`up_reply`、`up_dm`对象：

| 字段       | 类型 | 内容         | 备注                  |
| ---------- | ---- | ------------ | --------------------- |
| can_modify | bool | 是否可以修改 |                       |
| status     | num  | 状态         | 0：未开启<br />1：已开启 |

**示例：**

查询视频`oid=823136331`的评论主题交互状态

```shell
curl -G 'https://api.bilibili.com/x/v2/reply/subject/interaction-status' \
--data-urlencode 'type=1' \
--data-urlencode 'oid=823136331'
```
...
| 字段             | 类型 | 内容         | 备注 |
| ---------------- | ---- | ------------ | ---- |
| root_text        | str  | 根评论提示文字 |      |
| child_text       | str  | 子评论提示文字 |      |
| give_up_text     | str  | 放弃评论提示文字 |      |
| editor_icon_config | obj | 编辑器图标配置 |      |
| func_buttons     | obj  | 功能按钮配置 |      |

**示例：**

查询视频`oid=823136331`的评论主题描述信息

```shell
curl -G 'https://api.bilibili.com/x/v2/reply/subject/description' \
--data-urlencode 'type=1' \
--data-urlencode 'oid=823136331'
```
...
```json
{
  ...
  "data": {
    "base": {
      "count": "483",
      "input": {
        "root_text": "万水千山总是情，评论两句行不行",
        "child_text": "万水千山总是情，评论两句行不行",
        "give_up_text": "不发没关系，请继续友善哦~",
        "editor_icon_config": {
          "screenshot_icon_state": 1,
          "upload_picture_icon_state": 1,
          "vote_icon_state": 3,
          "note_icon_state": 1,
          "goods_icon_state": 3,
          "insert_content_icon_state": 1,
          "elec_icon_state": 3,
          "cm_biz_helper_icon_state": 3
        },
        "func_buttons": {
          "buttons": [
            {"type": 1, "state": 1},
