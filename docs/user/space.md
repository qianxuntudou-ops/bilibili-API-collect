...
| charging_pay | obj | 充电专属视频档位信息          | 非充电专属视频可能无此项 |
| bvid        | str  | 稿件bvid                       |                                                              |
| reason      | str  | 置顶视频备注                   |                                                              |
| inter_video | bool | 是否为合作视频                 | false：否<br />true：是                                      |
| is_charging_arc | bool | 是否为充电专属视频         |                                                              |
| elec_arc_type | num | 充电视频类型                 | 0：非充电专属视频<br />1：充电专属视频 |
| elec_arc_badge | str | 充电视频角标文字            | 例如`充电专属` |

`data`中的`charging_pay`对象：

| 字段  | 类型 | 内容         | 备注 |
| ----- | ---- | ------------ | ---- |
| level | num  | 充电档位代码 | 见[充电档位代码（`privilege_type`）与定价](../electric/monthly.md#充电档位代码privilege_type与定价) |
...
`list`中的`slist`数组中的对象：

| 字段         | 类型 | 内容         | 备注 |
| ------------ | ---- | ------------ | ---- |
| count        | num  | 该分组视频数 |      |
| name         | str  | 分组名称     | 例如`充电专属` |
| special_type | str  | 分组类型     | 例如`charging` |
...
| is_pay             | num  | 0              | 作用尚不明确                 |
| is_self_view       | bool | false          | 是否仅自己可见                 |
...
> https://api.bilibili.com/x/space/setting
...
| --------------- | ------- | -------------- | ---- |
| privacy         | obj     | 隐私设置       |      |
| show_nft_switch | boolean | 是否显示NFT开关 |      |
| index_order     | array   | 主页模块排序   |      |

`data`中的`privacy`对象：

| 字段                | 类型 | 内容                    | 备注                    |
| ------------------- | ---- | ----------------------- | ----------------------- |
| bangumi             | num  | 番剧隐私设置            | 0：公开，1：私密        |
| bbq                 | num  | BBQ隐私设置             | 0：公开，1：私密        |
| channel             | num  | 频道隐私设置            | 0：公开，1：私密        |
| charge_video        | num  | 充电视频隐私设置        | 0：公开，1：私密        |
| close_space_medal   | num  | 关闭空间勋章显示        | 0：显示，1：关闭        |
| coins_video         | num  | 投币视频隐私设置        | 0：公开，1：私密        |
| comic               | num  | 漫画隐私设置            | 0：公开，1：私密        |
| disable_following   | num  | 禁用关注列表查看        | 0：允许，1：禁用        |
| disable_show_fans   | num  | 禁用粉丝列表显示        | 0：显示，1：禁用        |
| disable_show_school | num  | 禁用学校信息显示        | 0：显示，1：禁用        |
| dress_up            | num  | 装扮隐私设置            | 0：公开，1：私密        |
| fav_video           | num  | 收藏视频隐私设置        | 0：公开，1：私密        |
| groups              | num  | 分组隐私设置            | 0：公开，1：私密        |
| lesson_video        | num  | 课程视频隐私设置        | 0：公开，1：私密        |
| likes_video         | num  | 点赞视频隐私设置        | 0：公开，1：私密        |
| live_playback       | num  | 直播回放隐私设置        | 0：公开，1：私密        |
| only_show_wearing   | num  | 仅显示穿戴的装扮        | 0：否，1：是            |
| played_game         | num  | 玩过游戏的隐私设置      | 0：公开，1：私密        |
| tags                | num  | 标签隐私设置            | 0：公开，1：私密        |
| user_info           | num  | 用户信息隐私设置        | 0：公开，1：私密        |

`data`中的`index_order`数组：
...
查询用户`mid=690820158`的空间设置

```shell
curl -G 'https://api.bilibili.com/x/space/setting' \
--data-urlencode 'mid=690820158'
```
...
> https://api.bilibili.com/x/space/reservation
...
| message | str  | 错误信息 | 默认为OK |
| ttl     | num  | 1        |      |
| data    | null | 数据本体 | 当前返回null |

**示例：**

查询用户`mid=690820158`的预约信息

```shell
curl -G 'https://api.bilibili.com/x/space/reservation' \
--data-urlencode 'vmid=690820158'
```
...
> https://api.bilibili.com/x/space/fav/nav
...
| pugv       | num   | 课程收藏数量   |      |
| note       | num   | 笔记数量       |      |
| topic_list | num   | 话题列表数量   |      |
| opus       | num   | 图文数量       |      |

**示例：**

查询用户`mid=8126584`的收藏夹导航信息

```shell
curl -G 'https://api.bilibili.com/x/space/fav/nav' \
--data-urlencode 'mid=8126584'
```
