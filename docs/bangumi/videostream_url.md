...
V2版本的番剧视频流获取接口，返回结构与V1版本类似，但包含更多业务信息。
...
| ------ | ---- | ---- | ------ | ---- |
| ep_id | num | 剧集epid | 必要 | |
| qn | num | 视频清晰度选择 | 非必要 | 默认为0 |
| fnval | num | 视频获取方式选择 | 非必要 | 默认为16 |
| fnver | num | 0 | 非必要 | 固定为0 |
| fourk | num | 是否允许4K视频 | 非必要 | 默认为1 |
| session | str | 会话标识 | 非必要 | |
| from_client | str | 客户端类型 | 非必要 | 如：BROWSER |
| is_main_page | bool | 是否主页面 | 非必要 | 默认为false |
| need_fragment | bool | 是否需要分段 | 非必要 | 默认为false |
| isGaiaAvoided | bool | 是否跳过GAIA验证 | 非必要 | |
| client_attr | num | 客户端属性 | 非必要 | 默认为0 |
| version_name | str | 版本名称 | 非必要 | |
| app_id | num | 应用ID | 非必要 | 如：100 |
| voice_balance | num | 音量平衡 | 非必要 | 默认为1 |
| drm_tech_type | num | DRM技术类型 | 非必要 | 默认为2 |
| scene | num | 场景代码 | 非必要 | 如：102 |
...
| 字段 | 类型 | 内容 | 备注 |
| ---- | ---- | ---- | ---- |
| play_view_business_info | obj | 播放视图业务信息 | |
| video_info | obj | 视频流信息 | 结构与V1版本类似 |

`play_view_business_info`对象：

| 字段 | 类型 | 内容 | 备注 |
| ---- | ---- | ---- | ---- |
| episode_info | obj | 分集信息 | |
| season_info | obj | 剧集信息 | |
| user_status | obj | 用户状态 | |
...
| 字段 | 类型 | 内容 | 备注 |
| ---- | ---- | ---- | ---- |
| follow_info | obj | 关注信息 | |
| is_login | num | 是否登录 | 1：已登录 |
| pay_info | obj | 支付信息 | |
| vip_info | obj | VIP信息 | |
| watch_progress | obj | 观看进度 | |

**示例：**

获取番剧`ep_id=3661125`的视频流

```shell
curl -G 'https://api.bilibili.com/pgc/player/web/v2/playurl' \
--data-urlencode 'ep_id=3661125' \
--data-urlencode 'qn=0' \
--data-urlencode 'fnval=16' \
--data-urlencode 'fourk=1' \
--data-urlencode 'from_client=BROWSER' \
-b 'SESSDATA=xxx'
```
