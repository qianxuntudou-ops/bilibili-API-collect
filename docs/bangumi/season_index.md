...
| area                 | num  | 地区         | 非必要 | 全部：`-1` &nbsp; [完整地区代码见表](info.md#剧集地区一览)<br>多个地区使用`,`分隔传递                        |
| is_finish            | num  | 状态         | 非必要 | 全部：`-1` &nbsp; 完结：`1` &nbsp; 连载：`0`                                                        |
| copyright            | num  | 版权         | 非必要 | 全部：`-1` &nbsp; 独家：`3` &nbsp; 其他：`1,2,4`                                                    |
| season_status        | num  | 付费类型     | 非必要 | 全部：`-1` &nbsp; 免费：`1` &nbsp; 付费：`2,6` &nbsp; 大会员：`4,6`                                 |
| season_month         | num  | 季度         | 非必要 | 全部：`-1` &nbsp; 一月：`1` &nbsp; 四月：`4` &nbsp; 七月：`7` &nbsp; 十月：`10`                     |
| year                 | num  | 年份         | 非必要 | 适用于`国创`和`番剧`类别<br>除了`-1`代表全部外，其他情况为起始年份（整数）                                           |
...
> https://api.bilibili.com/pgc/page/web/pop
...
> https://api.bilibili.com/pgc/season/episode/coin/user/number
...
| 字段       | 类型 | 内容         | 备注                  |
| ---------- | ---- | ------------ | --------------------- |
| is_original | num | 是否为原创   | 0：否<br />1：是      |
| number     | num  | 投币数量     |                       |

**示例：**

查询剧集分集`ep_id=740138`的投币数量

```shell
curl -G 'https://api.bilibili.com/pgc/season/episode/coin/user/number' \
--data-urlencode 'ep_id=740138'
```
...
| --------------- | ----- | -------------- | ---- |
| episode_id      | num   | 分集ID         |      |
| like_animation  | obj   | 点赞动画配置   |      |
| related_up      | array | 相关UP主列表   |      |
| stat            | obj   | 统计数据       |      |
| up_like_img     | obj   | UP主点赞图片   |      |
| user_community  | obj   | 用户社区信息   |      |
...
`data`中的`user_community`对象：

| 字段        | 类型 | 内容         | 备注                  |
| ----------- | ---- | ------------ | --------------------- |
| coin_number | num  | 用户投币数   |                       |
| favorite    | num  | 用户收藏状态 | 0：未收藏<br />1：已收藏 |
| is_original | num  | 是否原创     | 0：否<br />1：是      |
| like        | num  | 用户点赞状态 | 0：未点赞<br />1：已点赞 |

**示例：**

查询剧集分集`ep_id=740138`的网页信息

```shell
curl -G 'https://api.bilibili.com/pgc/season/episode/web/info' \
--data-urlencode 'ep_id=740138'
```
...
```json
{
  ...
  "data": {
    ...
    "related_up": [
      {
        "attribute": 0,
        "avatar": "https://i1.hdslb.com/bfs/face/20a871540518917ba7e9667b01ad47ebd905a7e9.gif",
        "is_follow": 0,
        "mid": 20124064,
        "uname": "想看就看一起看"
      }
...
> https://api.bilibili.com/pgc/season/web/related/recommend
...
| 字段    | 类型  | 内容           | 备注 |
| ------- | ----- | -------------- | ---- |
| relates | array | 相关推荐列表   |      |
| season  | array | 相关剧集列表   |      |

**示例：**

查询剧集`season_id=28747`的相关推荐

```shell
curl -G 'https://api.bilibili.com/pgc/season/web/related/recommend' \
--data-urlencode 'season_id=28747'
```
...
```json
{
  ...
  "data": {
    "relates": [
      {
        "desc1": "原著: 忘语、主笔: HeHe、蛋糕月、囧标、电驴",
        "desc2": "更新至390话",
        "item_id": 27591,
        "pic": "https://i0.hdslb.com/bfs/manga-static/c93802ed0918fd328d3afe316e25736c7b79de68.png",
        "title": "凡人修仙传",
        "type": 2,
        "type_name": "漫画",
        "url": "https://manga.bilibili.com/m/detail/mc27591?from=pgc"
      }
    ],
    "season": [
      {
        ...
        "cover": "https://i0.hdslb.com/bfs/bangumi/image/1a321192dd267265573b6f198d025d2da9e75ae1.png",
        "season_id": 45969,
        "title": "牧神记"
      }
    ]
...
| code    | num   | 返回值   | 0：成功                                                      |
| message | str   | 错误信息 | 默认为success                                                |
| data    | array | 卡片列表 | 当前返回空数组                                               |

**示例：**

查询剧集分集`ep_id=740138`的播放器OGV卡片

```shell
curl -G 'https://api.bilibili.com/pgc/season/player/ogv/cards' \
--data-urlencode 'ep_id=740138' \
