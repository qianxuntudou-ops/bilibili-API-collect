...
例如手机号为`13800138000`，国际id为`1 (中国大陆)`，登录秘钥为`aabbccdd`，极验challenge为`2333`，极验结果为`666666`，进行发送短信验证码操作

```shell
curl 'https://passport.bilibili.com/x/passport-login/sms/send' \
--data-urlencode 'tel=13800138000' \
...
使用手机号`13800138000`，短信验证码为`123456`，进行验证登录操作
